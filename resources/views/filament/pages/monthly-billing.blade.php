<x-filament-panels::page>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            ...
            <p class="text-xs text-gray-400 mt-1">{{ $totals['count'] }} report{{ $totals['count'] !== 1 ? 's' : '' }} received</p>
        </div>
        ...
        <div class="rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-950/30 p-4">
            ...
            <p class="text-xs text-green-500 mt-1">{{ $totals['paid_count'] }} tenant{{ $totals['paid_count'] !== 1 ? 's' : '' }}</p>
        </div>
        ...
        <div class="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-950/30 p-4">
            ...
            <p class="text-xs text-red-500 mt-1">{{ $totals['count'] - $totals['paid_count'] }} tenant{{ ($totals['count'] - $totals['paid_count']) !== 1 ? 's' : '' }}</p>
        </div>
        ...
        <div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-4">
            ...
            <p class="text-xs text-gray-400 mt-1">Subtotal £{{ number_format($totals['subtotal'], 2) }}</p>
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                <tr>
                    ...
                    <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-300">Scratch Orders</th>
                    <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-300">Other Orders</th>
                    <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-300">Subtotal</th>
                    <th class="px-4 py-3 text-right font-semibold text-gray-600 dark:text-gray-300">VAT</th>
                ...
                @foreach ($rows as $row)
                    ...
                    <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                        <td class="px-4 py-3">
                            ...
                            <div class="text-xs text-gray-400">{{ $tenant->tenant_key }}</div>
                        </td>

                        @if ($report)
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{{ number_format($report->scratchy_only_count) }}</td>
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-gray-300">{{ number_format($report->other_count) }}</td>
                            <td class="px-4 py-3 text-right text-gray-700 dark:text-gray-300">£{{ number_format($report->subtotal, 2) }}</td>
                            <td class="px-4 py-3 text-right text-amber-600 dark:text-amber-400">£{{ number_format($report->vat, 2) }}</td>
                            <td class="px-4 py-3 text-right font-semibold {{ $report->is_paid ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($report->is_paid)
                                    ...
                                    @if ($report->paid_at)
                                        <div class="text-xs text-gray-400 mt-0.5">{{ $report->paid_at->format('d/m/Y') }}</div>
                                    @endif
                                @else
                            ...
                            <td class="px-4 py-3 text-center">
                                @if ($report->is_paid)
                                    <x-filament::button
                                        wire:click="markUnpaid({{ $report->id }})"
                                        wire:confirm="Mark this as unpaid?"
                                        color="gray"
                                        size="xs"
                                    >
                                        Unmark
                                ...
                                @else
                                    <x-filament::button
                                        wire:click="markPaid({{ $report->id }})"
                                        wire:confirm="Mark this as paid?"
                                        color="success"
                                        size="xs"
                                    >
                                        Mark Paid
                        ...
                        @else
                            <td colspan="5" class="px-4 py-3 text-center text-gray-400 dark:text-gray-600 italic text-xs">
                                No report submitted
                            </td>
                            <td class="px-4 py-3 text-center">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-500">
                                    No data
                                </span>
                            </td>
            ...
            @if ($totals['count'] > 0)
                <tfoot class="bg-gray-50 dark:bg-gray-800 border-t-2 border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <td class="px-4 py-3 text-right font-semibold text-gray-700 dark:text-gray-300">£{{ number_format($totals['subtotal'], 2) }}</td>
                        <td class="px-4 py-3 text-right font-semibold text-amber-600 dark:text-amber-400">£{{ number_format($totals['vat'], 2) }}</td>
                        <td class="px-4 py-3 text-right font-bold text-gray-900 dark:text-white">£{{ number_format($totals['total'], 2) }}</td>
                        <td class="px-4 py-3 text-center text-xs text-gray-500 dark:text-gray-400">
                            {{ $totals['paid_count'] }}/{{ $totals['count'] }} paid
                        </td>
                        <td></td>
                    </tr>
                </tfoot>
