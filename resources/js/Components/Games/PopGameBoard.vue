...
// Winners that don't add to the money total (physical / no_auto_credit prizes)
...
const initializeItems = () => {
    ...
    popItems.value = activeTickets.value.map((ticket, index) => {
        ...
        // Check if this ticket's category is no_auto_credit
        const isNoAutoCredit = isWinner && instantWin && instantWin.category_id && props.instant_win_categories?.length
            ? props.instant_win_categories.some(c => c.id === instantWin.category_id && c.no_auto_credit)
            : false;

        const isTicketBundle = isWinner && instantWin && (instantWin as any).prize_type === 'ticket_bundle';
        ...
        return {
            ...
            isWinner,
            prize: isWinner && instantWin
                ? isTicketBundle
                    ? `${Math.floor(rawValue)} Free Ticket${rawValue !== 1 ? 's' : ''}`
                    : (siteCreditLabel((instantWin as any).prize_type, rawValue) ?? instantWin.prize)
                : null,
...
const popItem = async (item: PopItem, instant: boolean = false) => {
    ...
    // Handle win/loss. EVERY winner must celebrate — a £0-value physical prize
    // (e.g. a gold bar) or a no_auto_credit prize used to fall through to the
    // loss sound, telling a real winner they had lost. Only the credited total
    // stays gated on a positive auto-credited value.
    if (item.isWinner) {
        const isAutoCredited = item.prizeValue > 0 && !item.noAutoCredit;

        if (isAutoCredited) {
            totalWins.value += item.prizeValue;
        } else {
            otherPrizeWins.value++;
        }

        // Show win toast (only in non-instant mode to avoid spam)
        if (!instant) {
            winToastAmount.value = item.prizeValue;
            // Name-based toast for anything not auto-credited as money
            // (physical prizes, no_auto_credit) instead of a meaningless £0.00.
            winToastNoAutoCredit.value = !isAutoCredited;
            winToastPrizeName.value = item.prize || '';
...
const popAll = async () => {
    ...
    isPopAllRunning.value = false;
};

// Instantly resolve all remaining balloons without animation
const skipPopAll = () => {
    skipRequested.value = true;
    isPopAllRunning.value = false;

    for (const item of popItems.value) {
        if (item.popped || item.animating) continue;
        item.popped = true;
        item.animating = false;
        totalPopped.value++;
        emit('ticket-played', item.ticketId);
        ...
        if (item.isWinner) {
            if (item.prizeValue > 0 && !item.noAutoCredit) {
                totalWins.value += item.prizeValue;
            } else {
                otherPrizeWins.value++;
            }
...
<template>
    ...
    >
        ...
        <div class="actions">
            <button
                v-if="!allPopped && displayedItems.some(i => !i.popped) && !isPopAllRunning"
                class="pop-all-btn"
                @click="popAll"
            >
                Pop All
        ...
        <div v-if="allPopped" class="all-popped-message">
            <h3>All Done!</h3>
            <p v-if="otherPrizeWins > 0 && totalWins <= 0">You won <strong>{{ otherPrizeWins }} prize{{ otherPrizeWins > 1 ? 's' : '' }}</strong> — check My Prizes!</p>
            <p v-else-if="otherPrizeWins > 0">You won a total of <strong>{{ totalWins.toFixed(2) }}</strong> plus <strong>{{ otherPrizeWins }} prize{{ otherPrizeWins > 1 ? 's' : '' }}</strong>!</p>
            <p v-else>You won a total of <strong>{{ totalWins.toFixed(2) }}</strong></p>
        </div>

        <!-- Win Toast -->
        <Transition name="toast">
            <div v-if="showWinToast" class="win-toast">
                <span v-if="winToastNoAutoCredit" class="toast-amount">{{ winToastPrizeName }}</span>
                <template v-else>
